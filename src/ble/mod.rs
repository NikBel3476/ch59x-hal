...
fn configure_pa(pa_config: PaConfig) {
    ...
    unsafe {
        ...
        PA_CONFIG.txEnableGPIO = tx_pin.block().pa_out().as_ptr() as u32;
        PA_CONFIG.txDisableGPIO = tx_pin.block().pa_clr().as_ptr() as u32;

        PA_CONFIG.rx_pin = rx_pin._pin() as u32;
        PA_CONFIG.rxEnableGPIO = rx_pin.block().pa_out().as_ptr() as u32;
        PA_CONFIG.rxDisableGPIO = rx_pin.block().pa_clr().as_ptr() as u32;
...
pub unsafe extern "C" fn temperature_raw() -> u16 {
    ...
    let rb = &*pac::Adc::PTR;
    let tk = &*pac::TouchKey::PTR; // TODO: refine rb
    regs[0] = tk.tkey_cfg().read().bits();
    ...
    // restore regs
    tk.tkey_cfg().write(|w| w.bits(regs[0]));
